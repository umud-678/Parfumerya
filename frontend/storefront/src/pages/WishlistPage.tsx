    ...
    <PageShell title={t('wishlist.title')}>
      {items.length === 0 ? (
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          {items.map((product) => (
            <div key={product.id} className="card-elegant p-4 sm:p-6 flex gap-3 sm:gap-4 relative overflow-hidden">
              <img src={product.primaryImageUrl} alt={product.name} className="w-20 h-20 sm:w-24 sm:h-24 object-contain rounded-xl bg-plum-900/30 shrink-0" />
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  <button
                    ...
                    className="flex items-center gap-1 text-xs btn-primary px-3 py-2 min-h-[36px]"
                  >
